...
async function main() {
  try {
    ...
    // const ontoMed = loadProjectFromJson(path.resolve(process.cwd(),'examples/notWrongs/ontoMed/OntoMed_EngOntologias.json')); generateTptpFromProject(ontoMed, outputDir);
    // const resultOntoMed = await validateTptpFromProject(ontoMed, outputDir, true, false);console.log(resultOntoMed);
